...
随便哪个 modernx 的项目，只要敲入 npm start 就可以运行启动。之前敲了无数次我都没有在意，直到我准备研究源码的时候才意识到：**在敲下这行命令的时候，到底发生了什么呢？**
...
如果说 modernx 是亲女儿，那 [roadhog](https://github.com/sorrycc/roadhog.git) 就是亲哥哥了，起的是 webpack 自动打包和热更替的作用。
...
在 `src/index.js` 里，modernx 一共做了这么几件事：

0. 从 'modernx' 依赖中引入 modernx ：`import modernx from 'modernx'`; 

1. 通过函数生成一个 app 对象：`const app = modernx()`; 
...
在这 6 步当中，modernx 完成了 `使用 React 解决 view 层`、`redux 管理 model `、`saga 解决异步`的主要功能。事实上在我查阅资料以及回忆用过的脚手架时，发现目前端框架之所以被称为“框架”也就是解决了这些事情。前端工程师至今所做的事情都是在 **分离动态的 data 和静态的 view** ，只不过侧重点和实现方式也不同。
...
既然 modernx 是来自于 `modernx`，那么 modernx 是什么这个问题自然要去 modernx 的[源码](https://github.com/perlinson/modernx)中寻找了。

> 剧透：modernx 是个函数，返回一了个 app 的对象。

> 剧透2：目前 modernx 的源码核心部分包含两部分，`modernx` 和 `modernx-core`。前者用高阶组件 React-redux 实现了 view 层，后者是用 redux-saga 解决了 model 层。

老规矩，还是先翻 package.json 。

引用依赖很好的说明了 modernx 的功能：统一 view 层。

```json
// modernx 使用的依赖如下：

    "babel-runtime": "^6.26.0", // 一个编译后文件引用的公共库，可以有效减少编译后的文件体积
    "modernx-core": "^1.1.0", // modernx 另一个核心，用于处理数据层
    "global": "^4.3.2", // 用于提供全局函数的引用
    "history": "^4.6.3", // browserHistory 或者 hashHistory
    "invariant": "^2.2.2", // 一个有趣的断言库
    "isomorphic-fetch": "^2.2.1", // 方便请求异步的函数，modernx 中的 fetch 来源
...
```
不过 script 没有给太多有用的信息，因为 `ruban build` 中的 `ruban` 显然是个私人库(虽然在 tnpm 上可以查到但是也是私人库)。但根据惯例，应该是 modernx 包下的 `index.js` 文件提供了对外调用：
```js
...
显然这个 `exports.default` 就是我们要找的 modernx，但是源码中没有 `./lib` 文件夹。当然直接看也应该看不懂，因为一般都是使用 babel 的命令 `babel src -d libs` 进行编译后生成的，所以直接去看 `src/index.js` 文件。


## `src/index.js`

`src/index.js`[在此](https://github.com/perlinson/modernx/blob/master/packages/modernx/src/index.js) ：

在这里，modernx 做了三件比较重要的事情：

1. 使用 call 给 modernx-core 实例化的 app(这个时候还只有数据层) 的 start 方法增加了一些新功能（或者说，通过代理模式给 model 层增加了 view 层）。
2. 使用 react-redux 完成了 react 到 redux 的连接。
3. 添加了 redux 的中间件 react-redux-router，强化了 history 对象的功能。

### 使用 call 方法实现代理模式

modernx 中实现代理模式的方式如下：
...
- 因为 modernx-core 的 start 方法里有用到 this，不用 call 指定调用者为 app 的话，oldAppStart() 会找错对象。
...
```js
import connectAmodernxnced from '../components/connectAmodernxnced' 
export function createConnect({
  connectHOC = connectAmodernxnced,
.... 其他初始值
} = {}) {
...
// 1号 connect 的本体
export default function connectAmodernxnced() {
	// 逻辑处理
...
modernx 在此基础上又进行了一层代理，把代理后的对象当作初始值传递给了 modernx-core，方便其在 model 的 
subscriptions 中监听 router 变化。

看看 `index.js` 里 router 的实现：

1.在 createOpts 中初始化了添加 react-router-redux 中间件的方法和其 reducer ，方便 modernx-core 在创建 store 的时候直接调用。

2. 使用 patchHistory 函数代理 history.linsten，增加了一个回调函数的做参数(也就是订阅)。

> subscriptions 的东西可以放在 modernx-core 里再说，
...
import * as core from 'modernx-core';
...
`src/index.js` 主要实现了 modernx 的 view 层，同时传递了一些初始化数据到 modernx-core 所实现的 model 层。当然，还提供了一些 modernx 中常用的方法函数：

- `dynamic` 动态加载(2.0 以后官方提供 1.x 自己手动实现吧)
- `fetch` 请求方法(其实 modernx 只是做了一把搬运工)
- `saga`(数据层处理异步的方法)。

这么看 modernx 真的是很薄的一层封装。

而 modernx-core 主要解决了 model 的问题，包括 state 管理、数据的异步加载、订阅-发布模式的实现，可以作为数据层在别处使用(看 2.0 更新也确实是作者的意图)。使用的状体啊管理库还是 redux，异步加载的解决方案是 saga。当然，一切也都写在 index.js 和 package.json 里。
...
然后继续深扒 `modernx-core`，还是先从 `package.json` 扒起。

## package.json

`modernx-core` 的 `package.json` 中依赖包如下：
...
`src/index` 的源码在[这里](https://github.com/perlinson/modernx/blob/master/packages/modernx-core/src/index.js)

在 `modernx` 的 `src/index.js` 里，通过传递 2 个变量 `opts` 和 `createOpts` 并调用 `core.create`，`modernx` 创建了一个 app 对象。其中 `opts` 是使用者添加的控制选项，`createOpts` 则是初始化了 reducer 与 redux 的中间件。

`modernx-core` 的 `src/index.js` 里便是这个 app 对象的具体创建过程以及包含的方法：
...
> 我最开始很不习惯 JavaScript 就是因为 JavaScript 还是一个函数向的编程语言，也就是函数里可以定义函数，返回值也可以是函数，class 最后也是被解释成函数。在 modernx-core 里创建了 app 对象，但是把 model 和 start 的定义放在了后面。一开始对这种简写没看懂，后来熟悉了以后发现确实好理解。一眼就可以看到 app 所包含的方法，如果需要研究具体方法的话才需要向后看。

[Plugin](https://github.com/perlinson/modernx/blob/master/packages/modernx-core/src/Plugin.js) 是作者设置的一堆**钩子**性监听函数——即是在符合某些条件的情况下下(modernx 作者)进行手动调用。这样使用者只要按照作者设定过的关键词传递回调函数，在这些条件下便会自动触发。
...
`model` 是 app 添加 model 的方法，在 **modernx 项目** 的 index.js 是这么用的。

> app.model(require('./models/example'));

在 `modernx` 中没对 model 做任何处理，所以 `modernx-core` 中的 model 就是 **modernx 项目** 里调用的 model。
...
> Ever wonder why we dispatch the action like this in modernx ? `dispatch({type: 'example/loadDashboard'` 

## start 方法

`start` 方法是 `modernx-core` 的核心，在 `start` 方法里，modernx 完成了 **`store` 初始化** 以及 **`redux-saga` 的调用**。比起 `modernx` 的 `start`，它引入了更多的调用方式。
...
createPromiseMiddleware 的代码[在此](https://github.com/perlinson/modernx/blob/master/packages/modernx-core/src/createPromiseMiddleware.js)。
...
```js
  const middleware = () => next => (action) => {
    ...
    function isEffect(type) {
		// modernx 里 action 的 type 有固定格式： model.namespace/model.effects
...
[getSaga 源码](https://github.com/perlinson/modernx/blob/master/packages/modernx-core/src/getSaga.js)
...
1. modernx 通过 `app._getSaga(m.effects, m, onError, plugin.get('onEffect'))` 返回了一个 genenrator 函数。
...
不过 modernx 提供了自己的 `createStore` 方法，用来组织一系列自己创建的参数。
```js
    // Create store
...
combineReducers 中：
- 第一个 `...reducers` 是从 modernx 里传入的 historyReducer，以及通过 ` reducers[m.namespace] = getReducer(m.reducers, m.state);` 剥离出的 model 中的 reducer
- 第二个参数为手动在 plugin 里添加的 extraReducers；
- 第三个参数为异步 reducer，主要是用于在 modernx 运行以后动态加载 model 里的 reducer。


#### createStore


现在我们有了一个 combine 过的 reducer，有了 core 中创建的 sagaMiddleware 和 promiseMiddleware，还有了从 modernx 中传入的 createOpts，现在可以正式创建 store 了。

> 从 modernx 中传入的 createOpts 为 
```js
    setupMiddlewares(middlewares) {
      return [
        routerMiddleware(history),
...
现在 modernx 已经创建了 store，有了异步数据流加载方案，并且又做了一些其他的事情：
...
setupApp(app) 是从 modernx 里传过来的，主要是使用 patchHistory 函数代理 history.linsten，即强化了 redux 和 router 的联系，是的路径变化可以引起 state 的变化，进而听过监听 state 的变化来触发回调。
> 这也是 core 中唯一使用 this 的地方，逼得 modernx 中必须使用 oldStart.call(app) 来进行调用。
...
自此，model 中的四大组件全部完毕，完成了 modernx 的数据层处理。
