...
在modernx的官方仓库里，提供了上手教程，讲述了modernx的一些基本概念。到了真实的业务开发过程中，会遇到许许多多不能用那些基本操作覆盖的场景，本文尝试列举一些常见的需求在modernx中的实现方式。
...
注意到modernx中的每个model，实际上都是普通的JavaScript对象，包含
...
可以借助modernx社区的`modernx-model-extend`库来做这件事。
...
在redux-saga中，提供了take和takeLatest这两个操作，modernx是redux-saga的封装，也是可以使用这种操作的。
...
所以，我们也可以在modernx中使用take操作来监听action。
...
在modernx中，可以用namespace去指定接受action的model，所以可以通过类似这样的方式去组合：
...
对redux-saga的测试是这样的原理，而modernx是对redux-saga的封装，这块的机制是一致的，所以我们可以用同样的方式，从model对象中获取reducer和effect，分别编写测试用例。 
