...
modernx 是体验技术部开发的 React 应用框架，将上面三个 React 工具库包装在一起，简化了 API，让开发 React 应用更加方便和快捷。

modernx = React-Router + Redux + Redux-saga

## modernx 应用的最简结构
```js
import modernx from 'modernx';
const App = () => <div>Hello modernx</div>;
...
modernx 提供 app.model 这个对象，所有的应用逻辑都定义在它上面。
...
modernx 提供多个 effect 函数内部的处理函数，比较常用的是 `call` 和 `put`。
