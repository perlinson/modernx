...
```js
import { Router, Route, routerRedux } from 'modernx/router';
```
...
Util method to load React Component and modernx model dynamically.

e.g.

```js
import dynamic from 'modernx/dynamic';
...
* app: modernx instance
* models: function which return promise, and the promise return modernx model
* component：function which return promise, and the promise return React Component

## modernx API
### `app = modernx(opts)`

Create app, and return modernx instance. (Notice: modernx support multiple instances.)
...
e.g. register [modernx-loading](https://github.com/perlinson/modernx-loading) plugin:
...
const app = modernx({
  onReducer: reducer => {
    return (state, action) => {
      ...
      // 由于 modernx 同步了 routing 数据，所以需要把这部分还原
...
e.g. [modernx-loading](https://github.com/perlinson/modernx-loading) has implement auto loading state with this hook.

#### `onHmr(fn)`

HMR(Hot Module Replacement) related, currently used in [babel-plugin-modernx-hmr](https://github.com/modernxjs/babel-plugin-modernx-hmr).
...
e.g. use modernx with [redux-persist](https://github.com/rt2zz/redux-persist):
...
Recommend using separate file to config router. Then we can do hmr with [babel-plugin-modernx-hmr](https://github.com/modernxjs/babel-plugin-modernx-hmr). e.g.
...
## Model
model is the most important concept in modernx.
...
models's initial state, it's priority is lower than `opts.initialState` in `modernx()`.
...
View https://github.com/perlinson/modernx/blob/master/packages/modernx-core/test/reducers.test.js for details.
...
View https://github.com/perlinson/modernx/blob/master/packages/modernx-core/test/effects.test.js for details.
