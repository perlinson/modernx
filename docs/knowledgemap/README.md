...
- [Read "the modernx.js Knowledgemap" in English](https://github.com/perlinson/modernx-knowledgemap/blob/master/README_en.md)
- ["the modernx.js Knowledgemap" 日本語版](https://github.com/perlinson/modernx-knowledgemap/blob/master/README_ja.md)

---

不知大家学 react 或 modernx 时会不会有这样的疑惑：
...
这篇文档梳理了基于 [modernx-cli](https://github.com/perlinson/modernx-cli) 使用 [modernx](https://github.com/perlinson/modernx) 的最小知识集，让你可以用最少的时间掌握创建类似 [modernx-hackernews](https://github.com/perlinson/modernx-hackernews) 应用的全部知识，并且不需要掌握额外的冗余知识。
...
import { Link, Route } from 'modernx/router';
...
```javascript
const todos = ['Learn modernx'];
[...todos, 'Learn antd'];  // ['Learn modernx', 'Learn antd']
```
...
modernx 的 effects 是通过 generator 组织的。Generator 返回的是迭代器，通过 `yield` 关键字实现暂停功能。

这是一个典型的 modernx effect，通过 `yield` 把异步逻辑通过同步的方式组织起来。
...
```html
<h1 className="fancy">Hello modernx</h1>
```
...
数据处理在 React 中是非常重要的概念之一，分别可以通过 props, state 和 context 来处理数据。而在 modernx 应用里，你只需关心 props 。
...
```javascript
yield put({ type: 'todos/add', payload: 'Learn ModernX' });
```
...
modernx 里，effects 和 subscriptions 的抛错全部会走 `onError` hook，所以可以在 `onError` 里统一处理错误。
...
```javascript
import { routerRedux } from 'modernx/router';
...
先安装 modernx-cli@next 。

```bash
$ npm install modernx-cli@next -g
```
...
详见[和 modernx 一起用@umijs.org](https://umijs.org/guide/with-modernx.html)。
