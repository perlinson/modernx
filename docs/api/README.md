...
异步请求库，输出 [isomorphic-fetch](https://github.com/matthew-andrews/isomorphic-fetch) 的接口。不和 modernx 强绑定，可以选择任意的请求库。
...
* app: modernx 实例，加载 models 时需要
* models: 返回 Promise 数组的函数，Promise 返回 modernx model
...
创建应用，返回 modernx 实例。(注：modernx 支持多实例)
...
比如注册 [modernx-loading](https://github.com/perlinson/modernx-loading) 插件的例子：
...
封装 effect 执行。比如 [modernx-loading](https://github.com/perlinson/modernx-loading) 基于此实现了自动处理 loading 状态。

#### `onHmr(fn)`

热替换相关，目前用于 [babel-plugin-modernx-hmr](https://github.com/modernxjs/babel-plugin-modernx-hmr) 。
...
推荐把路由信息抽成一个单独的文件，这样结合 [babel-plugin-modernx-hmr](https://github.com/modernxjs/babel-plugin-modernx-hmr) 可实现路由和组件的热加载，比如：
...
## Model
model 是 modernx 中最重要的概念。以下是典型的例子：
...
初始值，优先级低于传给 `modernx()` 的 `opts.initialState`。
...
详见： https://github.com/perlinson/modernx/blob/master/packages/modernx-core/test/reducers.test.js
...
详见：https://github.com/perlinson/modernx/blob/master/packages/modernx-core/test/effects.test.js
